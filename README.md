...
| P (hold) + direction | **Pounce** — the dino coils (bigger = longer), leaps (faster = farther) and strikes EVERYTHING the leap crosses — the hit zones are live from the first inch of the jump to a beat after the landing, one strike per victim. Tail-fighters instead hold P to **tail-swing**: continuous hits on a metronome, but the feet are planted until you let go. |
